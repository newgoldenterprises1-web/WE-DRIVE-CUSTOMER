...
class _PickupDropScreenState extends State<PickupDropScreen> {
  ...
  Widget buildPremiumLanding() {
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.fromLTRB(16, 14, 16, 24), children: <Widget>[
        Container(
          padding: const EdgeInsets.all(20),
          decoration: BoxDecoration(gradient: const LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: <Color>[Color(0xFF0B2341), Color(0xFF173B6D)]), borderRadius: BorderRadius.circular(26), boxShadow: const <BoxShadow>[BoxShadow(color: Colors.black26, blurRadius: 16, offset: Offset(0, 6))]),
        ...
        SizedBox(height: 350, child: GridView.count(
  ...
  Widget sectionLabel(String title, String subtitle) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
      Text(title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w900, color: primary)),
  ...
  Widget buildTripType() {
    return Container(
      ...
      child: Row(children: <Widget>[
        ...
        if (tripType == 'Round Trip') Container(margin: const EdgeInsets.only(left: 4), padding: const EdgeInsets.only(left: 8), decoration: const BoxDecoration(border: Border(left: BorderSide(color: border))), child: Row(children: <Widget>[const Text('Stops', style: TextStyle(fontSize: 11, color: primary, fontWeight: FontWeight.w800)), Switch.adaptive(value: multipleStopsEnabled, onChanged: setMultipleStops, activeColor: gold, materialTapTargetSize: MaterialTapTargetSize.shrinkWrap)])),
      ]),
    );
  }

  Widget tripTypeButton(String label, IconData icon, bool selected, VoidCallback onTap) {
    return InkWell(onTap: onTap, borderRadius: BorderRadius.circular(13), child: AnimatedContainer(duration: const Duration(milliseconds: 180), padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 8), decoration: BoxDecoration(color: selected ? primary : Colors.transparent, borderRadius: BorderRadius.circular(13)), child: Row(mainAxisAlignment: MainAxisAlignment.center, children: <Widget>[Icon(icon, size: 17, color: selected ? Colors.white : primary), const SizedBox(width: 6), Text(label, style: TextStyle(color: selected ? Colors.white : primary, fontWeight: FontWeight.w800, fontSize: 11.5))])));
  }
  ...
  List<Widget> buildContent() {
    ...
    content.addAll(<Widget>[sectionLabel('Trip type', 'Choose how you want the chauffeur journey to work.'), const SizedBox(height: 9), buildTripType()]);
  ...
  Widget buildAirportOptions() {
    return Row(children: <Widget>[
      Expanded(child: optionCard('Drop to RGIA', 'Airport drop', Icons.flight_land_rounded, isToAirport, () { setState(() => isToAirport = true); handleAirportDefault(); })),
      const SizedBox(width: 10),
      Expanded(child: optionCard('Pickup from RGIA', 'Airport pickup', Icons.flight_takeoff_rounded, !isToAirport, () { setState(() => isToAirport = false); handleAirportDefault(); })),
    ]);
  }
    ...
    return FlutterMap(
      ...
      children: <Widget>[
        ...
        MarkerLayer(markers: <Marker>[
          ...
          for (final ll.LatLng p in <ll.LatLng>[ll.LatLng(pickupPos.latitude + 0.0035, pickupPos.longitude + 0.0028), ll.LatLng(pickupPos.latitude - 0.0029, pickupPos.longitude - 0.0032), ll.LatLng(pickupPos.latitude + 0.0018, pickupPos.longitude - 0.0041)]) Marker(point: p, width: 34, height: 34, child: Container(decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle, border: Border.all(color: primary, width: 1.5)), child: const Icon(Icons.person_pin, color: primary, size: 20))),
        ]),
      ],
    );
  }
  ...
  Widget buildRouteStats() {
    final String kmText = tripKm == null ? '--' : tripKm!.toStringAsFixed(1);
    final String minsText = tripMins?.toString() ?? '--';
    return Positioned(
      top: 14,
      left: 18,
      right: 18,
      child: Container(
        ...
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), boxShadow: const <BoxShadow>[BoxShadow(color: Colors.black26, blurRadius: 10, offset: Offset(0, 3))]),
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceAround,
          children: <Widget>[
            Text('$kmText km', style: const TextStyle(fontWeight: FontWeight.w900, color: primary, fontSize: 12.5)),
            const Text('|', style: TextStyle(color: border)),
            Text('$minsText mins', style: const TextStyle(fontWeight: FontWeight.w900, color: primary, fontSize: 12.5)),
            const Text('|', style: TextStyle(color: border)),
            const Text('Drivers nearby', style: TextStyle(fontWeight: FontWeight.w800, color: Colors.green, fontSize: 11.5)),
          ],
        ),
      ),
    );
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
          Text(widget.isPremium ? 'Premium ride' : 'Book your ride', style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w900)),
          Text(widget.isPremium ? 'Elevated chauffeur experience' : widget.serviceType, style: const TextStyle(fontSize: 10.5, color: Colors.white70, fontWeight: FontWeight.w600)),
        ]),
      ...
      bottomNavigationBar: SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(14, 8, 14, 14), child: Container(decoration: BoxDecoration(color: widget.isPremium ? deep : primary, borderRadius: BorderRadius.circular(20), boxShadow: const <BoxShadow>[BoxShadow(color: Colors.black26, blurRadius: 14, offset: Offset(0, 5))]), child: ElevatedButton(onPressed: confirmBooking, style: ElevatedButton.styleFrom(backgroundColor: Colors.transparent, shadowColor: Colors.transparent, minimumSize: const Size.fromHeight(62), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)), padding: const EdgeInsets.symmetric(horizontal: 16)), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: <Widget>[
