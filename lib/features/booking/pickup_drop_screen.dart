...
class _PickupDropScreenState extends State<PickupDropScreen> {
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      ...
                      Navigator.push(
                        context,
                        MaterialPageRoute<void>(
                          builder: (_) => PickupDropScreen(serviceType: schedule, isPremium: true),
                        ),
                      );
                    },
  ...
  Widget buildPremiumLanding() {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: const Text('Premium Chauffeur', style: TextStyle(fontWeight: FontWeight.bold)),
      ),
      body: ListView(
        padding: const EdgeInsets.all(18),
        children: <Widget>[
  ...
  Future<void> searchLocations(String value) async {
    try {
      ...
      final http.Response response = await http.get(
        Uri.parse(url),
        headers: const <String, String>{'User-Agent': 'WeDriveHyderabad/1.0'},
      ).timeout(const Duration(seconds: 4));
    ...
    final ll.LatLng? selected = await Navigator.push<ll.LatLng>(
      ...
      MaterialPageRoute<ll.LatLng>(
        builder: (_) => DropPicker(
          initial: initial,
          title: stopIndex != null ? 'Select Stop ${stopIndex + 1}' : pickup ? 'Select Pickup' : 'Select Destination',
        ),
      ),
    );
  ...
  List<Widget> buildContent() {
    ...
    if (tripType == 'Round Trip') {
      content.addAll(<Widget>[
        const SizedBox(height: 8),
        const Text(
          'Return to pickup after final drop. Enable Stops above to add up to 3 stops.',
                ...
                child: Padding(
                  ...
                  child: InkWell(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(color: outstationDays == days ? primary : Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: outstationDays == days ? primary : border)),
                      child: Center(
                        child: Text('$days Day${days > 1 ? 's' : ''}', style: TextStyle(color: outstationDays == days ? Colors.white : primary, fontWeight: FontWeight.bold, fontSize: 12)),
                      ),
                    ),
                  ),
                ),
      ...
      child: AnimatedContainer(
        ...
        child: Column(
          children: <Widget>[
            ...
            Text('₹${rate.toStringAsFixed(0)}', style: TextStyle(color: selected ? gold : Colors.grey.shade600, fontSize: 11.5, fontWeight: FontWeight.w700)),
          ],
        ),
      ),
    );
  ...
  Widget airportToggle(String label, bool selected, VoidCallback onTap) {
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: selected ? primary : Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: selected ? primary : border)),
        child: Center(
          child: Text(label, style: TextStyle(color: selected ? Colors.white : primary, fontWeight: FontWeight.bold, fontSize: 12.5)),
        ),
      ),
    );
  }
    ...
    return FlutterMap(
      ...
      children: <Widget>[
        ...
        if (routePoints.isNotEmpty)
          PolylineLayer(polylines: <Polyline>[Polyline(points: routePoints, strokeWidth: 5.5, color: const Color(0xFF1A73E8))]),
        MarkerLayer(
          markers: <Marker>[
            Marker(
              point: pickupPos,
            ...
            for (int i = 0; i < stopPositions.length; i++)
              if (stopPositions[i] != null)
                Marker(
                  point: stopPositions[i]!,
                  width: 40,
                  height: 40,
                  child: Container(
                    decoration: BoxDecoration(color: gold, shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 2)),
                    alignment: Alignment.center,
                    child: Text('${i + 1}', style: const TextStyle(color: primary, fontWeight: FontWeight.w900)),
                  ),
                ),
            if (dropPos != null)
              Marker(point: dropPos!, width: 44, height: 44, child: const Icon(Icons.location_on, color: Color(0xFFEA4335), size: 44)),
            for (final ll.LatLng p in driverPositions)
              Marker(
                point: p,
                width: 36,
                height: 36,
                child: Container(
                  decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle, border: Border.all(color: primary, width: 1.5)),
  ...
  Widget buildRouteStats() {
    return Positioned(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), boxShadow: const <BoxShadow>[BoxShadow(color: Colors.black12, blurRadius: 10, offset: Offset(0, 3))]),
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceAround,
          children: <Widget>[
            Text('$tripKm km', style: const TextStyle(fontWeight: FontWeight.bold, color: primary, fontSize: 13)),
            const Text('|', style: TextStyle(color: border)),
            Text('$tripMins mins', style: const TextStyle(fontWeight: FontWeight.bold, color: primary, fontSize: 13)),
            const Text('|', style: TextStyle(color: border)),
            const Text('Pilots Active', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.green, fontSize: 12)),
          ],
        ),
      ),
    );
        ...
        children: <Widget>[
          ...
          Positioned(
            ...
            child: FloatingActionButton.small(
              ...
              child: isLoading ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.my_location, size: 20),
            ),
          ),
          DraggableScrollableSheet(
            ...
            builder: (_, ScrollController controller) {
              return Container(
                decoration: const BoxDecoration(color: bg, borderRadius: BorderRadius.vertical(top: Radius.circular(24)), boxShadow: <BoxShadow>[BoxShadow(color: Colors.black12, blurRadius: 14, offset: Offset(0, -3))]),
                child: ListView(
                  controller: controller,
      ...
      bottomNavigationBar: SafeArea(
        child: Padding(
          ...
          child: ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(backgroundColor: primary, minimumSize: const Size.fromHeight(54), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))),
            child: Row(
              ...
              children: <Widget>[
                Column(
                  ...
                  mainAxisSize: MainAxisSize.min,
                  children: <Widget>[
                    Text('₹${fare.toStringAsFixed(0)}', style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w900, color: Colors.white)),
                    const Text('All-Inclusive • Zero Hidden Fees', style: TextStyle(fontSize: 10, color: Colors.white70, fontWeight: FontWeight.w600)),
                  ],
                ),
                const Row(
                  children: <Widget>[
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(
        children: <Widget>[
          FlutterMap(
            options: MapOptions(initialCenter: center, initialZoom: 15, onPositionChanged: (MapCamera camera, bool _) => center = camera.center),
            children: <Widget>[TileLayer(urlTemplate: 'https://mt1.google.com/vt/lyrs=m,traffic&x={x}&y={y}&z={z}', userAgentPackageName: 'com.wedrive.app')],
          ),
          ...
          Positioned(
            ...
            child: ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(backgroundColor: primaryColor, minimumSize: const Size.fromHeight(48), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))),
